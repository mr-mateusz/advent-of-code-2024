...
if __name__ == '__main__':
    ...
    for i in range(45):
        ...
        if int(result, 2) != x + y:
            print(f'Error on {i} bit: {x=}, {y=}, result={int(result, 2)}, expected={x + y}')
            ...
            for g in _gates_descriptions:
                if g[1] == 'XOR' and not g[3].startswith('z'):
                    if _values[g[3]] == 1:
                        ...
                        print(f'Z gate: {z_gate}')

                        print([(*z_gate[:-1], swap_candidate[-1]), (*swap_candidate[:-1], z_gate[-1])])
    ...
    _gates_descriptions = [g for g in _gates_descriptions if g not in _gates_to_swap] + _new_gates

    actual_result = run(_values_initial, _gates_descriptions)

    print(expected == int(actual_result, 2))
